...
__drv_dispatchType(IRP_MJ_PNP_POWER)
...
{
    ULONG ActualCount = 0;
    RtlUnicodeToUTF8N("", 0xffff, &ActualCount, str->Buffer, str->Length);
    ...
    IN PCUNICODE_STRING str)
{
    ...
    status = RtlUnicodeToUTF8N("", 0xffff,
        ...
        if (entry->u.Open.EaMdl) {
            entry->u.Open.EaBuffer =
                MmMapLockedPagesSpecifyCache(entry->u.Open.EaMdl,
                    UserMode, MmNonCached, NULL, TRUE, NormalPagePriority|MdlMappingNoExecute);
    ...
    ULONG *len) 
{
    ...
    __try {
#pragma warning(suppress: 28145) // No fields should be modified, with the exception of MDL_MAPPING_CAN_FAIL, which is used for drivers that need to be Microsoft Windows 98 or Windows NT (SP4) compatible
    ...
    IN PUNICODE_STRING NetRootName)
{
    ...
    if (NetRootName->Length == SrvCallName->Length + NfsPrefix.Length) {
		...
		//};
		UNICODE_STRING NetRootPrefix;
		NetRootPrefix.Buffer = &NetRootName->Buffer[SrvCallName->Length / 2];
		NetRootPrefix.Length = NfsPrefix.Length;
		NetRootPrefix.MaximumLength = NetRootName->MaximumLength - SrvCallName->Length;
...
//			(USHORT)rinfo->FileNameLength, rinfo->FileName };
		UNICODE_STRING dst;
		dst.Buffer = rinfo->FileName;
		dst.Length = (USHORT)rinfo->FileNameLength;
		dst.MaximumLength = (USHORT)rinfo->FileNameLength;
    ...
    case FileLinkInformation:
    {
		...
		UNICODE_STRING dst;
		dst.Buffer = linfo->FileName;
		dst.Length = (USHORT)linfo->FileNameLength;
		dst.MaximumLength = (USHORT)linfo->FileNameLength;
#ifdef DEBUG_FILE_SET
    ...
    case FileRenameInformation:
        {
            ...
            PFILE_RENAME_INFORMATION prinfo =
                (PFILE_RENAME_INFORMATION)RxContext->Info.Buffer;
//			const UNICODE_STRING dst = { dst(USHORT)prinfo->FileNameLength,
//				(USHORT)prinfo->FileNameLength, prinfo->FileName };
			UNICODE_STRING dst;
			dst.Buffer = prinfo->FileName;
			dst.Length = (USHORT)prinfo->FileNameLength;
			dst.MaximumLength = (USHORT)prinfo->FileNameLength;
...
//_IRQL_restores_ //paired ExAcquireFastMutex & ExReleaseFastMutex
void enable_caching(
    ...
    HANDLE session)
{
    ...
    if (!found && nfs41_fobx->deleg_type) {
        ...
        oentry = RxAllocatePoolWithTag(NonPagedPool, 
            sizeof(nfs41_fcb_list_entry), NFS41_MM_POOLTAG_OPEN);
		if (oentry == NULL) goto releasemutex;
    ...
    IN OUT PRX_CONTEXT RxContext)
{
	...
	if (!nfs41_fobx) goto out;
	if (!pNetRootContext) goto out;
...
#pragma warning(suppress: 6984) // In some circumstances, you might need to suppress this warning, such as drivers that  register dispatch routines inside a loop.
